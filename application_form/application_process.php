<?php
	...
	if($post){
		...
		$iswimreason = $_POST['iswimreason'];
		$workshop = stripslashes($_POST['workshop']);
		$workshopreason = $_POST['workshopreason'];
		$knew = stripslashes($_POST['knew']);
		...
		$message = '
			<html>
					...
					<body>
							...
							<p>Курс: '.$year.'</p>
							<p>Англ: '.$level.'</p>
							<p>Почему решил подать: '.$iswimreason.'</p>
							<p>Воркшоп: '.$workshop.'</p>
							<p>Почему этот воркшоп: '.$workshopreason.'</p>
							<p>Узнал нас от: '.$knew.'</p>
		...
		if(!$error){
			$mail = mail(APPLICATION_FORM, $subject, $message,
			     "From: ".$name." <".$email.">\r\n"
			    ."Reply-To: ".$email."\r\n"
